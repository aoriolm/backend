...
router.put("/citas/:id", (req, res) => {
    const { id } = req.params;
    const { fecha, hora, duracion, email, nombre, apellido1, apellido2, servicios, tel1, tel2 } = req.body;
    citaSchema
    .updateOne({ _id: id }, { $set: { fecha, hora, duracion, email, nombre, apellido1, apellido2, servicios, tel1, tel2 } })
    .then((data) => res.json(data))
    .catch((error) => res.json({ message: error}));
});
